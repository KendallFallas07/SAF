...
function obtenerDatos() {
  ...
  if (datos.length > 0) {
      let url = "../business/accionProveedorCreditoBuscar.php";
...
function establecerFechasMinimas() {
    const fechaAdquisicion = document.getElementById('FechaInicio');
...
function editData(event, id) {
    ...
    if (confirm("Desea Guardar Los cambios Realizados en el Proveedor Credito?")) {
        const form = document.getElementById(id);
        const formData = new FormData(form);

        fetch("../business/accionProveedorCreditoEditar.php", {
            method: form.method,
            body: formData
        })
        .then(response => {
        ...
        .then(data => {
            ...
            // Redirigir o actualizar el contenido según sea necesario
             Ejemplo: window.location.href = "./proveedorCredito.php";
        })
        .catch(error => {
...
function deleteP(identificadorProCre) {
    // Confirmación antes de eliminar
    if (!confirm("¿Estás seguro de que deseas eliminar este credito?")) {
        return;
    }

    fetch(`../business/accionProveedorCreditoBorrar.php?code=${encodeURIComponent(identificadorProCre)}`, {
        method: 'GET'
    })
            .then(response => response.json())
            .then(data => {
            ...
            .catch(error => {
                ...
                alert('Hubo un error al intentar eliminar el credito.');
            });
}
